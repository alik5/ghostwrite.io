class LetterTypesController < ApplicationController
  ...
  private
    ...
    def letter_type_params
      params.require(:letter_type).permit(:name, allow_destroy: true, :letter_fields_attributes => [ :id, :name, :short_name, :field_type, :if_business, :letter_type_id, allow_destroy: true ], :questions_attributes => [ :id, :name])
    end
end
